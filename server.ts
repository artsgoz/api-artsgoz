...
import serviceAccountKey from "./ids-artsgoz-firebase-adminsdk-zpxo3-d0e0f240fc.json" assert { type: "json" }
...
import Ajarn from "./Schema/Ajarn.js";
...
server.post("/add-staff", (req, res) => {

    let { fullname, username, dept } = req.body;

    let ajarn = new Ajarn({
        personal_info: { fullname, dept, username }
    })

    ajarn.save().then((a) => {

        return res.status(200).json(formatDatatoSendAjarn(a))
...
server.post("/search-ajarns", (req, res) => {

    let { query } = req.body;

    Ajarn.find({ "personal_info.username": new RegExp(query, 'i')})
    .select("personal_info.fullname personal_info.username personal_info.profile_img personal_info.dept -_id")
    .then(ajarns => {
        return res.status(200).json({ ajarns })
    })
    .catch(err => {
        return res.status(500).json({ error: err.message })
    })
...
server.post("/get-staff", (req, res) => {

    let { username } = req.body;

    Ajarn.findOne({ "personal_info.username": username })
    .select("-personal_info.fullname")
    .then(ajarn => {
        return res.status(200).json(ajarn)
    })
    .catch(err => {
        console.log(err);
        return res.status(500).json({ error: err.message })
